...
- PLATFORM - название системы в которой запущена игра
  ('Linux', 'Windows' и т.п.);
...
gfx.record_start(файл, [fps]) - начать запись экрана в
анимированный GIF, по умолчанию 20 кадров в секунду,
не дольше 2 минут. Вернёт true или false.

gfx.record_frame(пиксели) - добавить кадр в запись;
(Используется внутри движка, вручную звать не нужно!)

gfx.record_stop() - остановить запись и записать файл,
true если файл записан.

gfx.record_on() - идёт ли запись сейчас

В любом приложении запись переключается клавишей F12:
файл rein-ГГММДД-ЧЧММСС.gif создаётся в текущем каталоге.
Если приложение меняет размер экрана, текущий клип
сохраняется, а запись продолжается в новом файле
(имя-2.gif, имя-3.gif, ...).
...
:pixels(таблица) -- быстро нарисовать набор пикселей с
учётом альфы: {индекс, цвет, индекс, цвет, ...}, где
индекс = y*w + x + 1 (нумерация как у :buff), а цвет:

```
r*0x1000000 + g*0x10000 + b*0x100 + a
```

пиксели за границами изображения пропускаются
...
:resize(w, h, smooth) - вернёт новые пиксели после
изменения размера
