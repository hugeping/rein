...
net.tls(сокет, хост, [сертификат, ключ]) -- обернуть
соединение в TLS 1.2 (SNI = хост, сертификат сервера не
проверяется), вернёт tls сокет; сертификат и ключ клиента
(P-256) задаются в PEM, сертификат отправляется только
если сервер его запросил
...
net.certgen(имя) -- сделать самоподписанный сертификат P-256
с общим именем `имя`: вернёт сертификат и его ключ
(PKCS#8) в PEM; годится для net.tls

Есть библиотека sock, которая может быть включена в
приложение и использована в блокирующем режиме;
sock.dial(хост, порт, tls_host, [сертификат, ключ]) сразу
поднимает TLS.
