...
- ARGS -- массив с аргументами. Первый - путь к выполняемому скрипту;
...
- sys.exec(файл) -- запустить .lua файл как корутину, за exec обычно следует вызывать sys.stop или sys.suspend
...
- mixer.voices() -- загрузка инструментов;
- mixer.new(имя, данные) -- загрузка мелодии;
- mixer.volume(v) -- общая громкость;
- mixer.play(имя) -- проиграть мелодию (вернёт идентификатор);
- mixer.stop(идентификатор, [время затухания]) -- остановить мелодию.

# synth

Звуковая подсистема. Пока в работе.
