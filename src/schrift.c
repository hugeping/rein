...
/* nextafter taken from musl under the MIT License */
double nextaft(double x, double y)
{
...
clip_points(uint_fast16_t numPts, Point *points, int width, int height)
{
	...
	for (i = 0; i < numPts; ++i) {
		...
		if (pt.x >= width) {
			points[i].x = nextaft(width, 0.0);
		}
		if (pt.y < 0.0) {
			points[i].y = 0.0;
		}
		if (pt.y >= height) {
			points[i].y = nextaft(height, 0.0);
		}
	}
}
