...
describe("buf", function()
  ...
  it("remove_range compacts the text in place", function()
    local b = buf:new("x")
    b:set("abcdef")
    b:remove_range(3, 2)
    eq(b:gettext(), "abef")
  ...
  it("remove_range clamps the range to the text", function()
    local b = buf:new("x")
    b:set("abc")
    b:remove_range(5, 1)
    eq(b:gettext(), "abc")
  ...
  it("insert_range splices a chunk in place", function()
    local b = buf:new("x")
    b:set("abcd")
    b:insert_range(3, utf.chars("XY"))
    eq(b:gettext(), "abXYcd")
    b:insert_range(1, utf.chars("Z"))
    eq(b:gettext(), "ZabXYcd")
    b:insert_range(8, utf.chars("W"))
    eq(b:gettext(), "ZabXYcdW")
  end)
  ...
  it("save_atomic replaces the file and keeps the name", function()
    local fname = os.tmpname()
    local b = buf:new(fname)
    b:input("hello")
    ok(b:save_atomic())
    eq(b.fname, fname)
    ok(not b:dirty())
    ...
    eq(f:read('*a'), "hello")
    f:close()
    ok(not io.open(fname .. '.red'), "temporary file is renamed")
    local fname2 = os.tmpname()
    b:input(" world")
    ok(b:save_atomic(fname2))
    eq(b.fname, fname2)
    f = assert(io.open(fname2, "rb"))
    eq(f:read('*a'), "hello world")
    f:close()
    os.remove(fname)
    os.remove(fname2)
  end)
